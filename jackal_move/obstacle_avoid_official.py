...
class JackalNav:
    ...
    def reverse(self):
        ...
        move_cmd.linear.x = -2
        # For the next 6 seconds publish cmd_vel move commands to Turtlesim
        while min(self.data[280:440])<0.2:
    ...
    def obs_avoid(self):
        ...
        print('desviando')
        '''
        if min(self.right_setor) < min(self.left_setor):
        ...
        #if min(self.data[280:440]) == self.obstaculo:
            #if min(self.data[280:440]) <=0.2:
